...
const SignIn = () => {
   const [user, setuser] = useState({email:"",passwrod:""})
const [error, seterror] = useState(null);
let history = useHistory();

   const handleLogin=async(e)=>{
       e.preventDefault()
       const res =await fetch("http://localhost:5000/api/signin",{method:"POST",
            ...
            <div class="m-7">
                <form action="">
                    <div class="mb-6">
                        ...
                        <input type="email" name="email" id="email" placeholder="you@company.com" class="w-full px-3 py-2 
                    ...
                    <div class="mb-6">
                        <button type="button" class="w-full px-3 py-4 text-white bg-indigo-500 rounded-md 
                        focus:bg-indigo-600 focus:outline-none"
                        onClick={handleLogin}
                        >Sign in</button>
                    </div>
                    <p class="text-sm text-center text-gray-400">Don&#x27;t have an account yet? 
                    <Link to="/register" ><a class="text-indigo-400 focus:outline-none focus:underline focus:text-indigo-500
                     dark:focus:border-indigo-800">Sign up</a></Link>.</p>
                </form>
                {error && error.map(e=>{
                return(
                    <p class="text-lg font-extrabold text-red-500" >
                        {`${Object.getOwnPropertyNames(e)}:${e[Object.keys(e)[0]]}`}</p>
                )
            })}
            </div>
